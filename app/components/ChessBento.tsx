...
export function ChessBento() {
  const chessLink = "https://www.chess.com/member/mohammedgehad";
  const backRank = [
    "rook",
    "knight",
    "bishop",
  ...
  return (
    <BentoCard linkTo={chessLink} height="md:h-[304px] lg:h-[300px]">
        ...
        <div className="z-20 flex h-full flex-col">
          <div>
            ...
            <p className="mb-2 text-balance text-text-secondary">
              You might say I approach coding like a game of chess - strategic,
              methodical, and always thinking several moves ahead. But unlike
              chess, in programming, the pawns drink coffee, the knights debug,
              and the queen is a well-commented function that does all the work!
            </p>
          </div>
          <div className="mt-auto flex items-center justify-center">
            <div className="grid grid-cols-8 grid-rows-2 gap-2">
              ...
              {pawnRank.map((type, i) => (
                <ChessPiece key={`pawn-${i}`} type={type} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex h-full w-full flex-wrap">
        {Array.from({ length: 64 }).map((_, index) => {
          ...
          return (
            <div
              key={index}
              className={`w-1/8 h-1/8 ${isBlack ? "bg-slate-400" : "bg-transparent"}`}
...
function ChessPiece({ type }) {
  return (
    <div className="flex items-center justify-center opacity-80 transition-transform duration-300 hover:-translate-y-1 hover:text-indigo-500 hover:opacity-100">
      {BlackChessSVGs[type]}
    </div>
  );
}
